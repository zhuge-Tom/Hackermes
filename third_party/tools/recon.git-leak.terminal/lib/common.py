...
def setPaths(url):
    ...
    # Bundled data (user-agents.txt) ships beside the script, even when the
    # recovery output is redirected to the caller's working directory.
    paths.GITHACK_DATA_PATH = os.path.join(getattr(paths, "GITHACK_SCRIPT_ROOT", paths.GITHACK_ROOT_PATH), "data")
