...
def output_to_csv(global_data):
    ...
    while not global_data.empty():
        row = global_data.get()
        writer.writerow(row)
        # Hackermes compat patch: mirror each probed endpoint to stdout so the
        # ToolHost evidence (stdout only) carries the enumeration result.
        try:
            print("[SWAGGER] {} {} | status {}".format(str(row[3]).upper(), row[4], row[7]))
        except Exception as e:
            print(e)
