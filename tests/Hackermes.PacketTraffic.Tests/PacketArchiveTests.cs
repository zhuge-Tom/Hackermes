...
public sealed class PacketArchiveTests
{
    ...
    [Fact]
    public void HackermesJson_RejectsUnknownSchemaVersion()
    {
        var text = PacketArchiveCodec.Serialize([Sample], PacketArchiveFormat.HackermesJson)
            .Replace("\"version\": 1", "\"version\": 99", StringComparison.Ordinal);

        Assert.Throws<InvalidDataException>(() =>
            PacketArchiveCodec.Deserialize(text, PacketArchiveFormat.HackermesJson));
    }
    ...
    [Fact]
    public void HackermesJson_OldEntryWithoutBodyMetadataRemainsReadable()
    {
        const string legacy = """
        {
          "version": 1,
        ...
        var result = Assert.Single(PacketArchiveCodec.Deserialize(legacy, PacketArchiveFormat.HackermesJson));
