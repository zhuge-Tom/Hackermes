...
public sealed class PacketArchiveCodecTests
{
    ...
    [InlineData("capture.json", PacketArchiveFormat.HookmesJson)]
    [InlineData("capture", PacketArchiveFormat.HookmesJson)]
    ...
    [Fact]
    public void Hookmes_import_rejects_unknown_version_and_wrong_packet_kind()
    {
        Assert.Throws<InvalidDataException>(() => PacketArchiveCodec.Deserialize(
            "{\"version\":999,\"entries\":[]}", PacketArchiveFormat.HookmesJson));
        Assert.Throws<InvalidDataException>(() => PacketArchiveCodec.Deserialize(
