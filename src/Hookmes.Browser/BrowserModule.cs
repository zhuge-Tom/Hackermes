...
public sealed class BrowserModule : IModule
{
    ...
    /// <summary>
    /// 诊断入口:设置 <c>HOOKMES_AUTOOPEN_URL</c> 后启动即打开该地址。
    /// <para>
    /// 存在的理由是无人值守验证 —— CDP 通道能否建立只有真正跑起来才知道,
    /// 而手工点击无法纳入自动化检查。配合 <c>HOOKMES_SELFTEST=1</c> 可让标签页
    /// 在 CDP 就绪后自动跑一次自检并把结果写进日志。
    /// </para>
    /// </summary>
    private static void TryAutoOpen(IBrowserTabManager tabManager)
    {
        var url = Environment.GetEnvironmentVariable("HOOKMES_AUTOOPEN_URL");
