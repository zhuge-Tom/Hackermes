...
internal static class TrafficAnnotationToolRegistrar
{
    ...
    private static CommandResult ExecuteCommand(ITrafficAnnotationService service, CommandContext context)
    {
        try
        {
            var operation = context.Arg(0)?.ToLowerInvariant();
            if (operation == "list")
            {
                ...
                return CommandResult.Ok(Format(service.Query(query)));
            }
    ...
    {
        TrafficReviewStatus? status = args.TryGetProperty("status", out var statusElement)
