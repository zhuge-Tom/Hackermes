...
public enum PacketArchiveFormat { HookmesJson, Har }
...
public static class PacketArchiveCodec
{
    ...
    public static PacketArchiveFormat DetectFormat(string path) =>
        string.Equals(Path.GetExtension(path), ".har", StringComparison.OrdinalIgnoreCase)
            ...
            : PacketArchiveFormat.HookmesJson;
    ...
    public static string Serialize(IReadOnlyList<PacketArchiveEntry> entries, PacketArchiveFormat format) =>
        ...
        JsonSerializer.Serialize(new HookmesArchive(Version, entries), JsonOptions);

    public static IReadOnlyList<PacketArchiveEntry> Deserialize(string text, PacketArchiveFormat format) =>
        format == PacketArchiveFormat.Har ? DeserializeHar(text) : DeserializeHookmes(text);

    private static IReadOnlyList<PacketArchiveEntry> DeserializeHookmes(string text)
    {
        var archive = JsonSerializer.Deserialize<HookmesArchive>(text, JsonOptions)
    ...
    private static string SerializeHar(IReadOnlyList<PacketArchiveEntry> entries)
    {
        ...
        return JsonSerializer.Serialize(new HarRoot(new HarLog("1.2", new HarCreator("Hookmes", "1"), harEntries)), JsonOptions);
    }
    ...
    private sealed record HookmesArchive(int Version, IReadOnlyList<PacketArchiveEntry> Entries);
